...
@implementation ZYCardNoFormatter
...
- (void)cardNoField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    NSString *beingString = @"";
    if ([string isEqual: @""] && range.location > 0 && [[text substringWithRange:NSMakeRange(range.location, 1)] isEqualToString:self.separator]) {
        // 如果光标左侧是一个空格, 删除空格前一位数字
        beingString = [textField.text stringByReplacingCharactersInRange:NSMakeRange(range.location-1, 2) withString:string];
    } else {
        beingString = [textField.text stringByReplacingCharactersInRange:range withString:string];
    }
