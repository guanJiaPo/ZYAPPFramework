...
+ (NSDictionary *)getPropertys
{
    ...
    for (i = 0; i < outCount; i++) {
        ...
        } else if ([propertyType hasPrefix:@"Ti"]||[propertyType hasPrefix:@"TI"]||[propertyType hasPrefix:@"Ts"]||[propertyType hasPrefix:@"TS"]||[propertyType hasPrefix:@"TB"]||[propertyType hasPrefix:@"Td"]||[propertyType hasPrefix:@"TD"]) {
...
- (BOOL)update
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        DebugLog(res?@"更新成功":@"更新失败");
    }];
    return res;
}
/** 通过条件更新单个对象 */
- (BOOL)updateWithIdentifier:(NSString *)identifier{
    JKDBHelper *jkDB = [JKDBHelper shareInstance];
    __block BOOL res = NO;
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        NSString *tableName = NSStringFromClass(self.class);
        NSString *primaryValue = [self valueForKey:identifier];
        if ([NSString isEmpty:primaryValue]) {
            return ;
        }
        NSMutableString *keyString = [NSMutableString string];
        NSMutableArray *updateValues = [NSMutableArray  array];
        ...
        {
            NSString *proname = [self.columeNames objectAtIndex:i];
            if ([proname isEqualToString:identifier] || [proname isEqualToString:@"type"] || [proname isEqualToString:primaryId]) {
                continue;
            }
            [keyString appendFormat:@" %@=?,", proname];
            id value = [self valueForKey:proname];
        ...
        //删除最后那个逗号
        [keyString deleteCharactersInRange:NSMakeRange(keyString.length - 1, 1)];
        NSString *sql = [NSString stringWithFormat:@"UPDATE %@ SET %@ WHERE %@ = ?;", tableName, keyString, identifier];
...
/** 通过条件批量删除数据 */
+ (BOOL)deleteobjects:(NSArray *)array identifiers:(NSArray *)identifiers
{
    if (array.count == 0) {
        return NO;
    }
    ...
    [jkDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (JKDBModel *model in array) {
            NSString *tableName = NSStringFromClass(model.class);
            NSMutableString *keyString = [NSMutableString string];
            NSMutableArray *deleteValues = [NSMutableArray  array];
            for (NSString *identifier in identifiers) {
                id value = [model valueForKey:identifier];
                if (!value) {
                    continue;
                }
                [keyString appendFormat:@" %@=? and ", identifier];
                [deleteValues addObject:value];
            }
            //删除最后那个and加空格
            [keyString deleteCharactersInRange:NSMakeRange(keyString.length - 4, 4)];
            NSString *sql = [NSString stringWithFormat:@"DELETE FROM %@ WHERE %@",tableName,keyString];
            BOOL flag = [db executeUpdate:sql withArgumentsInArray:deleteValues];
            DebugLog(flag?@"删除成功":@"删除失败");
            if (!flag) {
                res = NO;
                *rollback = YES;
                return;
...
- (BOOL)isExsitWithObjectByIdentifier:(NSString *)identifier {
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        NSString *sql = [NSString stringWithFormat:@"SELECT FROM %@ WHERE %@ = ?",tableName,identifier];
...
+ (NSArray *)duplicateRemovalFindByCriteria:(NSString *)criteria {
